...
patch_apk() {
	...
	local cmd="java -jar $RV_CLI_JAR --rip-lib x86_64 --rip-lib x86 --rip-lib arm-v7a --rip-lib arm64-v8a --temp-dir=$tdir -c -a $stock_input -o $patched_apk -b $RV_PATCHES_JAR --keystore=ks.keystore $patcher_args"
	if [ "$OS" = Android ]; then
